...
public class CleanTerms {
   ...
   * 
   * @param synsList the list of synonyms
   * @param original the original term in the question
  ...
  public List<String> removeDuplicatedSynonyms(String original, List<String> synsList) {
    ...
    String stemmedOriginal = this.getStemmedTerm(original.toLowerCase());
    ...
    for (String s : synsList) {
      ...
      for (String s1 : sList) {
        newS = newS + " " + this.removeBeginingEndingHyphen(s1);
   ...
   * 
   * @param resources the list of all the synonyms
