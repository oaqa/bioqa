package edu.cmu.lti.oaqa.bio.retrieval.tools;
...
/**
 * The ExtractAbbrev class implements a simple algorithm for extraction of abbreviations and their
 * definitions from biomedical text. Abbreviations (short forms) are extracted from the input file,
 * and those abbreviations for which a definition (long form) is found are printed out, along with
 * that definition, one per line. A file consisting of short-form/long-form pairs (tab separated)
 * can be specified in tandem with the -testlist option for the purposes of evaluating the
 * algorithm.
 * 
 * @see <a href="http://biotext.berkeley.edu/papers/psb03.pdf">A Simple Algorithm for Identifying
 *      Abbreviation Definitions in Biomedical Text</a> A.S. Schwartz, M.A. Hearst; Pacific
 *      Symposium on Biocomputing 8:451-462(2003) for a detailed description of the algorithm.
 * @author Ariel Schwartz (modified a little)
 * @version 03/12/03
 */
public class ExtractAbbrev {
    ...
    private String extractAbbrPairReturn(String shortForm, String longForm) {
        String bestLongForm;
        StringTokenizer tokenizer;
        int longFormSize, shortFormSize;
        String result = "";
    ...
    /**
     * this one is to accept a question and return a pair of abbriviation and the long forms
     * 
     * @param sentence
     * @return
     */
    public List<String> extractAbbrPairsByString(String sentence) {
            ...
            do {
                ...
                if (shortForm.length() > 0 || longForm.length() > 0) {
                    if (shortForm.length() > 1 && longForm.length() > 1) {
                        ...
                        if (isValidShortForm(shortForm)) {
                            if (!extractAbbrPairReturn(shortForm.trim(), longForm.trim()).isEmpty())
                                results.add(extractAbbrPairReturn(shortForm.trim(), longForm.trim()));
                        }
                    }
                    currSentence = currSentence.substring(closeParenIndex + 1);
                } else if (openParenIndex > -1) {
    ...
    /**
     * get the abbreviation from the question. This one should be put in the key term
     * updater/extractor
     * 
     * @param question
     * @return a list of the combination of short form and long form (also as a list).
     */
    public HashMap<String, String> abbreviateExtractor(String question) {
        HashMap<String, String> hm = new HashMap<String, String>();
        ArrayList<String> abbreviations = (ArrayList) this.extractAbbrPairsByString(question);

        if (abbreviations.isEmpty())
            return null;

        for (String s : abbreviations) {
            String[] splits = s.split("\\:"); // split by comma because short form and long form are
                                              // combined with comma
            if(s.length() >= 2) {
                hm.put(splits[0], splits[1]);
                hm.put(splits[1], splits[0]); 
            }  
        }
        return hm;
    }
    ...
    public static void main(String[] args) {
        ...
        // parse arguments
        /*
         * for (int i = 0; i < args.length; i++) { if (args[i].equals("-testlist")) { if (i ==
         * args.length - 1) { usage(); } testList = args[++i]; extractAbbrev.testMode = true; } else
         * if (args[i].equals("-usage")) { usage(); } else if (args[i].equals("-help")) { usage(); }
         * else { filename = args[i]; // Must be last arg if (i != args.length - 1) { usage(); } } }
         * if (filename == null) { usage(); }
         */
        if (extractAbbrev.testMode)
            extractAbbrev.loadTrueDefinitions(testList);
        // extractAbbrev.extractAbbrPairs("test.txt");
        System.out.println(extractAbbrev
                        .extractAbbrPairsByString("How do Cathepsin D (CTED) and apolipoprotein E (ApoE) interactions contribute to Alzheimer's disease?"));
        if (extractAbbrev.testMode)
            System.out.println("TP: " + extractAbbrev.truePositives + " FP: "
                            + extractAbbrev.falsePositives + " FN: " + extractAbbrev.falseNegatives
                            + " TN: " + extractAbbrev.trueNegatives);
    }
}
