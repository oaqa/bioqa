...
public class QueryGenerator {
   ...
   * 
   *         public String generateIndriQuery(boolean filter, HashMap<String, Boolean> map) { return
   *         this.generateIndriQuery(filter, map, ""); }
   */
  /**
   ...
   * @param filter Whether to use the boolean-filter in Indri or not
   * @param map The map contains all the resource selections
   * @param field The field to search in Indri. Possible options are "[legalspan]", "[sentence],
   *        etc.
   * @return formed Indri query
   * 
   *         public String generateIndriQuery(boolean filter, HashMap<String, Boolean> map, String
   *         field) {
   * 
   *         QueryStrategy refiner = new QueryStrategy(this.keyTerms);
   * 
   *         if(map.isEmpty()) { this.setDefaultResourceFilter(); map = this.resourceFilter; } else
   ...
   * 
   *         String query = ""; // general query String mainPart = ""; // boolean-filter part
   * 
   *         for (QueryComponent q : qc.getQueryComponent()) {
   * 
   *         String keyterm = "";
   * 
   *         // wraps the keyterm if it is a phrase with "#od2". Using "2" here is based on
   *         experiment. keyterm = q.getKeyterm().getText().contains(" ") ? "#od2(" +
   *         q.getKeyterm().getText() + ")" : q.getKeyterm().getText();
   * 
   *         // wraps the keyterm and synonyms String tempMain = q.getSynonyms().isEmpty() ? " " +
   *         keyterm : " #syn( " + keyterm + q.getSynonymsToString("#od1(", ")") + ")";
   * 
   *         query = query + " " + q.getWeight() + tempMain; // the general query
   * 
   *         if (q.isConcept()) mainPart = mainPart + " " + tempMain; // content in boolean-filter
   * 
   *         }
   * 
   *         // forms the query String s = mainPart.isEmpty() ? "#weight" + field + "( " + query +
   *         " ) " : "#filreq( #band (" + mainPart + ")" + "#weight" + field + "( " + query + " ) "
   *         + ")";
   * 
   *         // returns the general query or boolean-filter query if (!filter) return "#weight" +
   *         field + "( " + query + " ) "; else return s; }
   */
