...
public class TermProximityRanker extends ContentAwarePassageUpdater {
  ...
  private double updatePassageScore(Map<String, Double> keytermCount,
    ...
    if (numQueryTerms > 1) {
      ...
      // System.out.println(c1 + "\t" + numQueryTerms + "\t" + c2 + "\t" + sumDists + "\t"
      // + (c2 - sumDists / combinatorial(numQueryTerms)) + "\t" + (c2 / c1) + "\t"
      // + totalNumQueryTerms + "\t" + c3 + "\t" + R);
    } else {
      R = c1 * numQueryTerms + totalNumQueryTerms * c3;
      // System.out.println(c1 + "\t" + numQueryTerms + "\t" + c2 + "\t" + c2 + "\t" + 0 + "\t"
      // + (c2 / c1) + "\t" + totalNumQueryTerms + "\t" + c3 + "\t" + R);
    }
    return R;
  }
