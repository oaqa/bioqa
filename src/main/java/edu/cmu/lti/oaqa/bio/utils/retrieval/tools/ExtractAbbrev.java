...
public class ExtractAbbrev {
  ...
  private String findBestLongForm(String shortForm, String longForm) {
    ...
    for (; sIndex >= 0; sIndex--) {
      ...
      while (((lIndex >= 0) && (Character.toLowerCase(longForm.charAt(lIndex)) != currChar))
              || ((sIndex == 0) && (lIndex > 0) && (Character.isLetterOrDigit(longForm
                      .charAt(lIndex - 1)))))
        lIndex--;
      if (lIndex < 0)
        return null;
  ...
  public static void main(String[] args) {
     ...
     * args.length - 1) { usage(); } testList = args[++i]; extractAbbrev.testMode = true; } else if
     * (args[i].equals("-usage")) { usage(); } else if (args[i].equals("-help")) { usage(); } else {
     * filename = args[i]; // Must be last arg if (i != args.length - 1) { usage(); } } } if
     * (filename == null) { usage(); }
     */
    ...
    if (extractAbbrev.testMode)
      System.out.println("TP: " + extractAbbrev.truePositives + " FP: "
              + extractAbbrev.falsePositives + " FN: " + extractAbbrev.falseNegatives + " TN: "
